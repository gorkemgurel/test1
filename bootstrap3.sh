...
install_prebuilt_sdk() {
  ...
  for ((i=0; $i < $CONFNUM; i++)); do
    ...
    if [ "DEBUG" = "DEBUG" ] && [ ! -f "${SDK_CHECK_PATH}" ]; then
      echo "ERROR: Failed to find compilation support SDK in the downloaded archive!"
      return 1
    fi

    "${MKDIR}" "${PROJECT_PATH}/VoodooInput/Debug" || ret=$?
    if [ $ret -ne 0 ]; then
      echo "ERROR: Failed to create SDK directory ${PROJECT_PATH}/VoodooInput/Debug with code ${ret}!"
      return 1
    fi

    "${MV}" "${SDK_PATH}" "${PROJECT_PATH}/VoodooInput/Debug/${SDK_PATH}" || ret=$?
    if [ $ret -ne 0 ]; then
    ...
    if [ -d "${DSYM_PATH}" ]; then
      "${MV}" "${DSYM_PATH}" "${PROJECT_PATH}/VoodooInput/Debug/${DSYM_PATH}" || ret=$?
      if [ $ret -ne 0 ]; then
    ...
    echo "Installed prebuilt SDK DEBUG/${vers}!"
  done
...
install_compiled_sdk() {
  ...
  for ((i=0; $i < $CONFNUM; i++)); do
    ...
    if [ ! -d "build/Debug/${SDK_PATH}" ]; then
      echo "ERROR: Failed to find the built SDK!"
      return 1
    fi

    if [ "DEBUG" = "DEBUG" ] && [ ! -f "build/Debug/${SDK_CHECK_PATH}" ]; then
    ...
    "${MV}" "build/Debug/${SDK_PATH}" "${PROJECT_PATH}/VoodooInput/Debug/${SDK_PATH}" || ret=$?
    if [ $ret -ne 0 ]; then
      echo "ERROR: Failed to install SDK with code ${ret}!"
      return 1
    fi

    if [ -d "build/Debug/${DSYM_PATH}" ]; then
      "${MV}" "build/Debug/${DSYM_PATH}" "${PROJECT_PATH}/VoodooInput/Debug/${DSYM_PATH}" || ret=$?
      if [ $ret -ne 0 ]; then
        echo "ERROR: Failed to install SDK dSYM with code ${ret}!"
        return 1
      fi
